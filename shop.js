(() => {
  ...
  let serial = 0;
  const defaults = () => ({currency_name:'Credits',starting_balance:'0',page_size:'6',allow_free_offers:false,currency_admin_uids:''});
  ...
  function importText(text) {
    ...
    String(text).replace(/^\uFEFF/,'').split(/\r?\n/).forEach((raw,i)=>{
      ...
      if(Object.hasOwn(settings,key)&&key!=='currency_admin_uids'){
        if(key==='allow_free_offers'){if(!['true','false'].includes(v))throw Error(`${key}: true or false required.`);settings[key]=v==='true';}else settings[key]=v;return;
      }
  ...
  $('shop-form').addEventListener('input',ev=>{
    ...
    else if(el.dataset.key){const target=o||state.settings;target[el.dataset.key]=['enabled','allow_free_offers'].includes(el.dataset.key)?el.value==='true':el.value;}
